...
pub fn read_configfile<P: AsRef<Path>>(path: P) -> Data {
    ...
    let yaml_map: serde_yaml::Mapping =
        serde_yaml::from_reader(filestream).expect("Couldn't parse feeds list");
    ...
    data.title = yaml_map
        ...
        .unwrap()
        .to_string();
    data.subtitle = yaml_map
        .get(&serde_yaml::Value::String("Subtitle".to_string()))
        .unwrap()
        .as_str()
        .unwrap()
        .to_string();

    data.entries_per_page = yaml_map
        .get(&serde_yaml::Value::String("Entries_per_page".to_string()))
        .unwrap()
        .as_i64()
        .unwrap() as u32;
    data.entries_in_atom = yaml_map
        .get(&serde_yaml::Value::String("Entries_in_atom".to_string()))
        .unwrap()
        .as_i64()
        .unwrap() as u32;

    let feeds_seq = yaml_map
        .get(&serde_yaml::Value::String("Feeds".to_string()))
        .unwrap()
        .as_sequence()
        .unwrap();
...
fn parse_feed(yml_feed: &serde_yaml::Value) -> FeedInfo {
    ...
    let home_key = &serde_yaml::Value::String("homepage".to_string());
        ...
        .get(url_key)
        .expect("value feedurl")
        .as_str()
        .expect("url str")
        .to_string();
    fi.homepage = feed_map
        .get(home_key)
        .expect("value homepage")
        .as_str()
        .expect("home str")
        .to_string();
