...
pub fn merge_entries<P: AsRef<Path>>(entries: &mut Vec<Entry>, filepath: P) {
    if !filepath.as_ref().exists() {
        ...
        zipw.start_file("entries.json", zip::write::FileOptions::default())
            .expect("Cant start file (create)");
        zipw.write(&serialized.into_bytes())
            .expect("Cant write entries (create)");
        ...
        let serialized: Vec<Entry> =
            serde_json::from_reader(zipfile).expect("cant read store file");
